...
import healthIntegrationRoutes from './routes/healthIntegrationRoutes.js';
import careerRoutes from './routes/careerRoutes.js';
import careerIntegrationRoutes from './routes/careerIntegrationRoutes.js';
...
app.get('/auth/google/start', (req, res) => {
  ...
  if (!clientId) {
    return res.status(400).send('NOTIFICATION_MAIL_CLIENT_ID environment variable is missing.');
  }
...
app.get('/auth/google/callback', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).send(`OAuth2 Exchange Failed: ${error.response?.data?.error_description || error.message}`);
  }
});
